...
if (isset($_POST['signIn'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($user_password, $user['user_password'])) {
            ...
            // Record whether they logged in as a buyer or a seller
            $user_type = $_POST['user_type'] ?? '';
            if (in_array($user_type, ['buyer', 'seller'], true)) {
                $type_stmt = $conn->prepare("UPDATE users SET user_type = ? WHERE user_id = ?");
                $type_stmt->bind_param("si", $user_type, $user['user_id']);
                $type_stmt->execute();
