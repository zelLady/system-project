...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fullname === '' || $email === '' || $mobile === '' || $password === '') {
        ...
        if ($check->get_result()->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (user_name, user_email, user_password, user_contactnum, user_type) VALUES (?, ?, ?, ?, 'buyer')");
            $stmt->bind_param("ssss", $fullname, $email, $hashedPassword, $mobile);

            if ($stmt->execute()) {
                $user_id = $stmt->insert_id;

                // Create an empty shop row too, so the schema stays consistent
                // with every other user (buyers can still browse/checkout fine).
                $shop_stmt = $conn->prepare("INSERT INTO shop (user_id, shop_name) VALUES (?, ?)");
                $defaultName = $fullname . "'s Shop";
                $shop_stmt->bind_param("is", $user_id, $defaultName);
                $shop_stmt->execute();

                echo "<script>alert('Registration successful! Please log in.'); window.location.href='sample1.php';</script>";
                exit();
            } else {
...
<head>
	...
	<title>FruzzHub Sample 2 (Buyer's View)</title>
...
<body>
	...
	<div class="main">
		...
		<div class="login_form">
			<form action="sample2.php" method="post" id="signupForm" novalidate>
				<h1>REGISTER</h1>
				...
				<p>Already have an account? <a href="sample1.php">Sign In</a></p>
			</form>
		</div>
